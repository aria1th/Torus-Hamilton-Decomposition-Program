...
flowchart LR
  ...
  O --> M --> A --> S --> D --> F --> L --> T --> P
  P --> R38
  P --> R42 --> BQ --> BE --> RG --> BT --> R96 --> RT --> RM --> RA --> RP --> Q --> NG --> OPEN
```
...
13. R42 promotion readiness is explicitly audited.  It is not promotion-ready:
    pointwise first-return equations, no-early/minimality, and a Lean-facing
    endpoint theorem are still missing.
14. R42 pointwise law mining now quantifies the missing trace grammar: the
...
15. The open residue queue is now explicit: R42 remains the active affine
