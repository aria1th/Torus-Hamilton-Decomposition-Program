...
5. **D5 even Route-E.**  Find a finite residue-branch menu of count/slot
   ...
   `nu=(r,0,0,h+r,r)` and a two-block seam map.  Its count-sum and weighted
   return-time arithmetic are now recorded in Lean, so the remaining B20 work
   is the symbolic trace theorem: first-return equations, no-earlier-return,
   pointwise return-time partition, and seam rank/single-cycle.  The canonical
