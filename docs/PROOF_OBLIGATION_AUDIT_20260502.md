...
4. **D5 even Route-E.**  Find count/slot families covering every even
   ...
   section-level lemma, not as the final all-even return theorem.  For the
   canonical open-port triple, the immediate Lean gap is the finite carry step
   law named by `RouteEOpenPortCanonicalChartStepTarget`.
