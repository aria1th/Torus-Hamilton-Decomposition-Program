...
- D5 even seam compatibility is automatic in Lean, but the newer
  `d5_even_routeE_bundle_v0_1.zip` changes the live even track.  D5 even
  should now be pursued as a Route-E one-`Lambda_E` periodic-excursion
  certificate problem rather than only as the older seam-SAT search.
...
- `scripts/search_4plus2_kappa_formulas.py` separates a first fiber-compiler
  ...
  for `79`, and `1` for `37`.
  The `--section-trace-diagnostics` output then checks the bundled witness
  itself along the section-return traces: for bundled `m=9`, color `0` has only
  `5/55` pure classes for `slot_by_layer_p_z_component`, and adding full
  coordinate residues modulo `3` gives only `11/1235` pure classes.  So the
  finite witness is not merely using a different affine dihedral formula; along
  the first trace it already depends on finer base-state information.
...
- `scripts/verify_compact_4plus2_formula_certs.py --target-a --product`
  ...
  `certs/d7_4plus2_compact_formula_witnesses.json` for `m=11,13,17`; with
  `--rank-summary-dir`, it also emits formula-defined base/fiber rank-step
  fingerprints from the C++ checker.
