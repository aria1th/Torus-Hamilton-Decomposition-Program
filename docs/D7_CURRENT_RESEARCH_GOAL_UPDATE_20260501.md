...
- `D7Odd/Handoff/Additive4Plus2ConcreteGoal.lean` specializes that target to
  ...
  return-to-base and base orbit coverage automatically.
...
1. Provide a uniform `BridgeConcreteRankPackage` for every odd `m >= 5`;
   after the latest interface, this means row permutations, D3
   fiber-layer/permutation data, a bijective base rank stepped by the canonical
   folded base return, and fiber monodromy for the canonical folded return.
