...
| B20 boundary one-cycle adapter | `RouteEB20.BoundaryQuotientCycleEnumeration.singleCycle`, `RouteEB20.BoundaryQuotientCycleEnumeration.oneCycleTarget` | adapter from bijective `Fin (boundaryCycleLength q)` enumeration plus semiconjugacy to `BoundaryQuotientOneCycleTarget`; concrete enumeration instance still open |
