...
| B20 boundary one-cycle adapter | `RouteEB20.boundaryCycleNode`, `RouteEB20.BoundaryQuotientCycleEnumeration.singleCycle`, `RouteEB20.BoundaryQuotientCycleEnumeration.oneCycleTarget` | combined rank-indexed enumeration function plus adapter from bijective `Fin (boundaryCycleLength q)` enumeration and semiconjugacy to `BoundaryQuotientOneCycleTarget`; step/bijection proofs still open |
