...
| Q=1 plus-set-family split | `PrefixCount.PMOneBase.sum_if_mem_one_neg_one`; `PrefixCount.PMOneBase.sum_if_mem_one_neg_one_eq_neg_one_of_card_half`; `PrefixCount.PMOneBase.exists_finset_card_eq_and_mem`; `PrefixCount.PMOneBase.PlusFamily`; `PrefixCount.PMOneBase.PlusFamily.rowPlusSet`; `PrefixCount.PMOneBase.PlusFamily.rowMateSet`; `PrefixCount.PMOneBase.PlusFamily.toBase_entry_nonneg_iff`; `PrefixCount.PMOneBase.PlusFamily.base_row_sum`; `PrefixCount.PMOneBase.PlusFamily.nonempty`; `PrefixCount.PMOneBase.PlusFamily.toBase`; `PrefixCount.PMOneBase.PlusFamily.toMatching`; `PrefixCount.PMOneBase.upgrade_row_sum`; `PrefixCount.PMOneBase.mate_indicator_sum_eq_one`; `PrefixCount.PMOneBase.upgrade_row_sum_of_mate`; `PrefixCount.PMOneBase.PlusFamily.upgraded_row_sum`; `PrefixCount.PMOneBase.PlusFamily.upgraded_row_sum_of_mate`; `PrefixCount.MarginTransportQeq1PlusFamilyGoal`; `PrefixCount.marginTransportQeq1MatchedPMOneGoal_of_plusFamily`; `oddCoreHighModulusPrefixCountGoal_of_qge2Plan_qeq1PlusFamily_and_geometry`; `odd_modulus_tori_all_dimensions_of_qge2Plan_qeq1PlusFamily_geometry_and_small_packet_lift` | Lean-checked in `RoundComposite/PrefixCount.lean` and `RoundComposite/OddCore.lean` | Conditional adapter |
...
`PlusFamily.base_row_sum`, `PMOneBase.upgrade_row_sum`, and
`PMOneBase.upgrade_row_sum_of_mate`, reducing the remaining row-margin choice
to plus-membership counts plus the matched-row indicator.  The still-open q=1
work is to choose plus sets and row margins whose upgraded row sums match
`P.sigma` and satisfy the zero-row restriction.
