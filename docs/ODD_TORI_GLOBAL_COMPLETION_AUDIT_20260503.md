...
For the `q >= 2` branch, the safest current target is the compatible margin
transport interface:
...
This is equivalent to the direct `MarginTransportQge2Goal` interface, but it
exposes exactly the low-delta restrictions that must be constructed.  The
preferred global endpoint in that shape is
`odd_modulus_tori_all_dimensions_of_qge2Compat_qeq1PlusFamily_rootFlatCanonical_and_small_packet_lift`.
