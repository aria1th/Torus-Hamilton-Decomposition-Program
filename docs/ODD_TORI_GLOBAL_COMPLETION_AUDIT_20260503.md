...
| Q>=2/q=1 compatible margin target | `PrefixCount.StepNonnegCompatibility.of_step_nonneg`; `PrefixCount.MarginTransportQge2CompatibleGoal`; `PrefixCount.MarginTransportQeq1CompatibleGoal`; `PrefixCount.marginTransportQge2Goal_of_compatible`; `PrefixCount.marginTransportQeq1Goal_of_compatible`; `PrefixCount.marginTransportQge2CompatibleGoal_of_margin`; `PrefixCount.marginTransportQeq1CompatibleGoal_of_margin`; `oddCoreHighModulusPrefixCountGoal_of_qge2Compat_qeq1Compat_and_geometry`; `oddCoreHighModulusPrefixCountGoal_of_qge2Compat_qeq1Compat_and_rootFlatCanonical`; `odd_modulus_tori_all_dimensions_of_qge2Compat_qeq1Compat_geometry_and_small_packet_lift`; `odd_modulus_tori_all_dimensions_of_qge2Compat_qeq1Compat_rootFlatCanonical_and_small_packet_lift` | Lean-checked in `RoundComposite/PrefixCount.lean` and `RoundComposite/OddCore.lean` | Preferred conditional adapter |
| Q>=2 direct margin plus-family adapter | `PrefixCount.MarginTransportQge2Goal`; legacy `oddCoreHighModulusPrefixCountGoal_of_qge2Margin_qeq1PlusFamily_and_geometry`; legacy `oddCoreHighModulusPrefixCountGoal_of_qge2Margin_qeq1PlusFamily_and_rootFlatCanonical`; legacy `odd_modulus_tori_all_dimensions_of_qge2Margin_qeq1PlusFamily_geometry_and_small_packet_lift`; legacy `odd_modulus_tori_all_dimensions_of_qge2Margin_qeq1PlusFamily_rootFlatCanonical_and_small_packet_lift` | Lean-checked in `RoundComposite/OddCore.lean` | Legacy conditional adapter only; the q=1 plus-family partner is Lean-proved impossible globally |
| Q>=2 global-`Qge2PlanBounds` split | `PrefixCount.Qge2PlanBounds.not_for_q_two_r_one`; `PrefixCount.MarginTransportQge2PlanGoal`; legacy `oddCoreHighModulusPrefixCountGoal_of_qge2Plan_qeq1PlusFamily_and_geometry`; legacy `oddCoreHighModulusPrefixCountGoal_of_qge2Plan_qeq1PlusFamily_and_rootFlatCanonical`; legacy `odd_modulus_tori_all_dimensions_of_qge2Plan_qeq1PlusFamily_geometry_and_small_packet_lift`; legacy `odd_modulus_tori_all_dimensions_of_qge2Plan_qeq1PlusFamily_rootFlatCanonical_and_small_packet_lift` | Lean-checked in `RoundComposite/PrefixCount.lean` and `RoundComposite/OddCore.lean` | Compatibility adapter only; too strong for universal q>=2 because `q = 2, r = 1` is incompatible with global `Qge2PlanBounds`, and the old plus-family q=1 partner is not a preferred target |
| Q>=2 plan/matrix split | `PrefixCount.MarginPlanQge2Goal`; `PrefixCount.SignedMarginMatrixForQge2PlanGoal`; `PrefixCount.marginTransportQge2PlanGoal_of_plan_and_matrix`; legacy `oddCoreHighModulusPrefixCountGoal_of_qge2PlanParts_qeq1PlusFamily_and_geometry`; legacy `odd_modulus_tori_all_dimensions_of_qge2PlanParts_qeq1PlusFamily_geometry_and_small_packet_lift` | Lean-checked in `RoundComposite/PrefixCount.lean` and `RoundComposite/OddCore.lean` | Compatibility adapter only; too broad as preferred target unless plan feasibility is strengthened, and the old q=1 plus-family partner is not global |
| Q=1 transport compatibility split | `PrefixCount.quotient_eq_one_range_of_mqr`; `PrefixCount.quotient_eq_one_m_eq_pred_add`; `PrefixCount.MarginPlan.delta_eq_zero_iff`; `PrefixCount.MarginPlan.delta_eq_one_iff`; `PrefixCount.MarginPlan.tau_le_q`; `PrefixCount.StepNonnegCompatibility`; `PrefixCount.StepNonnegCompatibility.of_step_nonneg`; `PrefixCount.StepNonnegCompatibility.step_nonneg`; `PrefixCount.MarginTransportQeq1CompatibleGoal`; `PrefixCount.marginTransportQeq1Goal_of_compatible`; `PrefixCount.marginTransportQeq1CompatibleGoal_of_margin`; `oddCoreHighModulusPrefixCountGoal_of_qge2Plan_qeq1Compat_and_geometry`; `odd_modulus_tori_all_dimensions_of_qge2Plan_qeq1Compat_geometry_and_small_packet_lift` | Lean-checked in `RoundComposite/PrefixCount.lean` and `RoundComposite/OddCore.lean` | Conditional adapter |
...
| Q=1 plus-set-family split | `PrefixCount.PMOneBase.sum_if_mem_one_neg_one`; `PrefixCount.PMOneBase.sum_if_mem_one_neg_one_eq_neg_one_of_card_half`; `PrefixCount.PMOneBase.exists_finset_card_eq_and_mem`; `PrefixCount.PMOneBase.PlusFamily`; `PrefixCount.PMOneBase.PlusFamily.rowPlusSet`; `PrefixCount.PMOneBase.PlusFamily.rowMateSet`; `PrefixCount.PMOneBase.PlusFamily.rowMateSet_card_le_one`; `PrefixCount.PMOneBase.PlusFamily.rowMateSet_card_eq_one_of_mate`; `PrefixCount.PMOneBase.PlusFamily.rowMateSet_card_eq_zero_of_not_mate`; `PrefixCount.PMOneBase.PlusFamily.exists_rowMateSet_card_eq_zero`; `PrefixCount.PMOneBase.PlusFamily.toBase_entry_nonneg_iff`; `PrefixCount.PMOneBase.PlusFamily.base_row_sum`; `PrefixCount.PMOneBase.PlusFamily.nonempty`; `PrefixCount.PMOneBase.PlusFamily.toBase`; `PrefixCount.PMOneBase.PlusFamily.toMatching`; `PrefixCount.PMOneBase.upgrade_row_sum`; `PrefixCount.PMOneBase.mate_indicator_sum_eq_one`; `PrefixCount.PMOneBase.upgrade_row_sum_of_mate`; `PrefixCount.PMOneBase.PlusFamily.upgraded_row_sum`; `PrefixCount.PMOneBase.PlusFamily.upgraded_row_sum_of_mate`; `PrefixCount.PMOneBase.PlusFamily.upgraded_row_sum_of_not_mate`; `PrefixCount.PMOneBase.PlusFamily.upgraded_row_sum_ne_zero_of_not_mate`; `PrefixCount.PMOneBase.PlusFamily.not_all_upgraded_row_sum_zero`; `PrefixCount.not_marginTransportQeq1PlusFamilyGoal`; legacy `PrefixCount.MarginTransportQeq1PlusFamilyGoal`; legacy `PrefixCount.marginTransportQeq1MatchedPMOneGoal_of_plusFamily`; legacy plus-family endpoints | Lean-checked in `RoundComposite/PrefixCount.lean` and `RoundComposite/OddCore.lean` | Diagnostic/legacy adapter only; global plus-family goal is Lean-proved impossible |
| Root-flat geometric split | `PrefixCountRootState`; `PrefixCountRootFlatReturnGoal`; `PrefixCountRootFlatCayleyLiftGoal`; `RootFlatCayleyStepCompatible`; `PrefixCountRootFlatEquivLiftGoal`; `PrefixCountRootFlatCanonicalReturnGoal`; `standardCayleySolved_of_rootFlatLayeredEquiv`; `prefixCountRootFlatCayleyLiftGoal_of_equiv`; `prefixCountGeometricCriterionGoal_of_rootFlat`; `prefixCountRootLayerEquivSucc`; `prefixCountRootStepSucc`; `prefixCountRootLayerEquivSucc_step`; `standardCayleySolved_of_rootFlatLayered_standardStepSucc`; `prefixCountRootLayerEquiv`; `prefixCountRootStep`; `prefixCountRootStep_eq_succ_cast`; `prefixCountRootLayerEquiv_step`; `standardCayleySolved_of_rootFlatLayered_standardStep`; `prefixCountGeometricCriterionGoal_of_rootFlatCanonical`; `oddCoreHighModulusPrefixCountGoal_of_qge2Compat_qeq1Compat_and_rootFlatCanonical`; `odd_modulus_tori_all_dimensions_of_qge2Compat_qeq1Compat_rootFlatCanonical_and_small_packet_lift`; legacy `rootFlatEquiv`, `qge2Margin`, `qge2Plan`, and `qge2PlanParts` endpoints | Lean-checked in `RoundComposite/OddCore.lean` | Conditional adapter; canonical-step lift closed |
...
| Active-Hall symboling foundation for small branch | `ActiveHall.Incidence`; `ActiveHall.Incidence.colorDegree`; `ActiveHall.Incidence.cutCap`; `ActiveHall.Incidence.hitCount`; `ActiveHall.Incidence.sum_colorDegree`; `ActiveHall.Incidence.sum_colorDegree_on`; `ActiveHall.Incidence.cutCap_symbols_univ`; `ActiveHall.Incidence.cutCap_colors_univ`; `ActiveHall.Incidence.cutCap_colors_empty`; `ActiveHall.Incidence.cutCap_symbols_empty`; `ActiveHall.Incidence.cutCap_symbol_singleton`; `ActiveHall.Incidence.cutCap_mono`; `ActiveHall.Incidence.cutCap_mono_colors`; `ActiveHall.Incidence.cutCap_mono_symbols`; `ActiveHall.Incidence.exists_injective_token_matching_of_hall`; `ActiveHall.CountMatrix`; `ActiveHall.CountMatrix.cutMass`; `ActiveHall.CountMatrix.cutMass_symbols_univ`; `ActiveHall.CountMatrix.cutMass_colors_univ`; `ActiveHall.CountMatrix.cutMass_symbols_univ_eq_cutCap`; `ActiveHall.CountMatrix.cutMass_colors_univ_eq_cutCap`; `ActiveHall.CountMatrix.cutMass_colors_empty`; `ActiveHall.CountMatrix.cutMass_symbols_empty`; `ActiveHall.CountMatrix.cutMass_colors_empty_eq_cutCap`; `ActiveHall.CountMatrix.cutMass_symbols_empty_eq_cutCap`; `ActiveHall.CountMatrix.cutMass_symbol_singleton`; `ActiveHall.CountMatrix.cutMass_mono`; `ActiveHall.CountMatrix.cutMass_mono_colors`; `ActiveHall.CountMatrix.cutMass_mono_symbols`; `ActiveHall.CountMatrix.hallCuts_of_nontrivial`; `ActiveHall.CountMatrix.hallCuts_iff_nontrivial`; `ActiveHall.CountMatrix.singleSymbol_hall`; `ActiveHall.CountMatrix.exists_singleSymbol_token_matching`; `ActiveHall.CountMatrix.HallCuts`; `ActiveHall.ResidueSpec`; `ActiveHall.CountMatrix.HasResidues`; `ActiveHall.CountMatrix.rowCompatible_of_hasResidues`; `ActiveHall.CountMatrix.colCompatible_of_hasResidues`; `ActiveHall.FeasibleWithResidues`; `ActiveHall.FeasibleWithResidues.rowCompatible`; `ActiveHall.FeasibleWithResidues.colCompatible`; `ActiveHall.FeasibleWithResidues.compatible`; `ActiveHall.Symboling`; `ActiveHall.Symboling.count`; `ActiveHall.Symboling.count_row_sum`; `ActiveHall.Symboling.count_col_sum`; `ActiveHall.Symboling.toCountMatrix`; `ActiveHall.Symboling.Realizes`; `ActiveHall.Symboling.HasResidues`; `ActiveHall.Symboling.hasResidues_of_realizes`; `ActiveHall.Symboling.toCountMatrix_hallCuts`; `ActiveHall.SymbolingWithResidues`; `ActiveHall.SymbolingWithResidues.feasible`; `ActiveHall.SymbolingWithResidues.rowCompatible`; `ActiveHall.SymbolingWithResidues.colCompatible`; `ActiveHall.SymbolingWithResidues.compatible`; `ActiveHall.HallRealizationGoal`; `ActiveHall.symbolingWithResidues_of_feasible_and_realization`; `ActiveHall.feasibleWithResidues_of_symbolingWithResidues` | Lean-checked in `RoundComposite/ActiveHall.lean`; imported by `RoundComposite.lean` | Closed foundation and conditional realization adapter |
| Active-Hall realization request | `docs/ACTIVE_HALL_REALIZATION_REQUEST_20260503.md` | Documents exact finite theorem, warning against naive Hall matching, and requested proof/counterexample outputs | Open proof request |
| Active-Hall realization brute-force sanity check | `scripts/active_hall_realization_bruteforce.py` | Default run checks six small `(T,C,X)` ranges and finds no counterexample | Audit/regression evidence only |
...
Once `hHigh` and `hSmallPacket` are formalized, the current dispatcher yields
the target all-dimensional odd-modulus theorem without using the finite
boundary table.
...
For the high branch, the safest current target is the compatible margin
transport interface for both quotient regimes:
...
This is equivalent to the direct margin interfaces after
`StepNonnegCompatibility.of_step_nonneg`, but it exposes exactly the low-delta
restrictions that must be constructed.  The preferred global endpoint in that
shape is
`odd_modulus_tori_all_dimensions_of_qge2Compat_qeq1Compat_rootFlatCanonical_and_small_packet_lift`.
...
`tau i <= 0` while `tau_sum = q-r = 1`; this obstruction is Lean-recorded as
...
not empty.  It now also records row-level formulas
`PlusFamily.base_row_sum`, `PlusFamily.upgraded_row_sum`, and
`PlusFamily.upgraded_row_sum_of_mate`, reducing the remaining row-margin choice
to plus-membership counts plus the matched-row indicator.  Lean also records
`PlusFamily.upgraded_row_sum_ne_zero_of_not_mate` and
`PlusFamily.not_all_upgraded_row_sum_zero`, so the `q = 1, r = 1` boundary
cannot be treated as a trivial all-zero signed-row case under the current pure
plus-family upgrade.  The still-open q=1 work is to choose plus sets and row
margins whose upgraded row sums match `P.sigma` and satisfy the zero-row
restriction, or to refine the q=1 correction interface for that boundary.

There is no longer a preferred global endpoint at this layer.  Lean proves
`PrefixCount.not_marginTransportQeq1PlusFamilyGoal` by instantiating
`d = m = 5`, `q = r = 1`; therefore the plus-set-family construction is a
diagnostic and legacy adapter only, not the q=1 theorem family.
...
is retained only as a legacy compatibility adapter.
...
endpoint is now
`odd_modulus_tori_all_dimensions_of_qge2Compat_qeq1Compat_rootFlatCanonical_and_small_packet_lift`.
The older `rootFlatEquiv`, `qge2Margin`, `qge2Plan`, and `qge2PlanParts`
endpoints remain available as compatibility adapters.
