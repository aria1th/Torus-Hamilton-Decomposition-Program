...
| Active-Hall symboling foundation for small branch | `ActiveHall.Incidence`; `ActiveHall.Incidence.colorDegree`; `ActiveHall.Incidence.cutCap`; `ActiveHall.CountMatrix`; `ActiveHall.CountMatrix.cutMass`; `ActiveHall.CountMatrix.HallCuts`; `ActiveHall.ResidueSpec`; `ActiveHall.CountMatrix.HasResidues`; `ActiveHall.CountMatrix.rowCompatible_of_hasResidues`; `ActiveHall.CountMatrix.colCompatible_of_hasResidues`; `ActiveHall.Symboling`; `ActiveHall.Symboling.count`; `ActiveHall.Symboling.count_row_sum`; `ActiveHall.Symboling.count_col_sum`; `ActiveHall.Symboling.toCountMatrix`; `ActiveHall.Symboling.Realizes`; `ActiveHall.Symboling.HasResidues`; `ActiveHall.Symboling.hasResidues_of_realizes`; `ActiveHall.Symboling.toCountMatrix_hallCuts` | Lean-checked in `RoundComposite/ActiveHall.lean`; imported by `RoundComposite.lean` | Closed foundation |
