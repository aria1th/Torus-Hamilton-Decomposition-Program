...
- `RoundComposite/PrefixCount.lean`: foundational high-modulus prefix-count
  ...
  also has the dense-matrix bridge `Parts.toMatrix`, column-splitting through
  ...
  The quotient/remainder branch split for `q = m/(d-1)` and
  `r = m%(d-1)` is packaged by `quotient_remainder_count_branch`, and
...
- `RoundComposite/ActiveHall.lean`: active Hall-symboling foundation for the
  ...
  count matrices, residue specifications, Hall cuts, realization, and proves
...
- `docs/ODD_TORI_CURRENT_GOAL_V3_1_20260504.md`: current concise goal reset
  ...
  q=1 degree-matrix special matching, root-flat canonical return, and the
  small-modulus slack-packet lift.  The q=1 auxiliary degree-matrix existence
  theorem is Lean-closed separately.
