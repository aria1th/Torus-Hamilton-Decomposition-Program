...
def nonbufferTokenBaseRight
    {b m T : Nat} [NeZero m] [NeZero (m ^ b)]
    {packets : List (List Nat)}
    {A : OddSuccessorPhaseSplitBufferReservoirData
      (b := b) (m := m) (T := T) packets}
    (hTpos : 0 < T) (a : A.nonbufferTokenBase) : Fin T :=
  Fin.cast (by omega) a.2.succ

def bufferTokenBaseRight
    {T : Nat} (hTpos : 0 < T) (ρ : Fin (T - 1)) : Fin T :=
  Fin.cast (by omega) ρ.succ

def nonzeroSymbolPred {T : Nat} (σ : Fin T) (hσ0 : σ.val ≠ 0) :
    Fin (T - 1) :=
  ⟨σ.val - 1, by omega⟩

@[simp] theorem nonbufferTokenBaseRight_eq_token_right
    {b m T : Nat} [NeZero m] [NeZero (m ^ b)]
    {packets : List (List Nat)}
    {A : OddSuccessorPhaseSplitBufferReservoirData
      (b := b) (m := m) (T := T) packets}
    (hTpos : 0 < T) (a : A.nonbufferTokenBase) (k : Fin (m - 1)) :
    BaseTail.Trades.NonbufferReservoirToken.right hTpos (a, k) =
      nonbufferTokenBaseRight (A := A) hTpos a := rfl

@[simp] theorem bufferTokenBaseRight_eq_token_right
    {m T : Nat} (hTpos : 0 < T)
    (k : Fin (m - 1)) (ρ : Fin (T - 1)) :
    BaseTail.Trades.BufferReservoirToken.right hTpos (k, ρ) =
      bufferTokenBaseRight hTpos ρ := rfl

@[simp] theorem bufferTokenBaseRight_nonzeroSymbolPred
    {T : Nat} (hTpos : 0 < T) (σ : Fin T) (hσ0 : σ.val ≠ 0) :
    bufferTokenBaseRight hTpos (nonzeroSymbolPred σ hσ0) = σ := by
  ext
  simp [bufferTokenBaseRight, nonzeroSymbolPred]
  omega

@[simp] theorem nonbufferTokenBaseRight_nonzeroSymbolPred
    {b m T : Nat} [NeZero m] [NeZero (m ^ b)]
    {packets : List (List Nat)}
    {A : OddSuccessorPhaseSplitBufferReservoirData
      (b := b) (m := m) (T := T) packets}
    ...
    (σ : Fin T) (hσ0 : σ.val ≠ 0) :
    nonbufferTokenBaseRight (A := A) hTpos
        (c, nonzeroSymbolPred σ hσ0) = σ := by
  ext
...
def zeroReservoirDelta
    {b m T : Nat} [NeZero m] [NeZero (m ^ b)]
    {packets : List (List Nat)}
    {A : OddSuccessorPhaseSplitBufferReservoirData
      (b := b) (m := m) (T := T) packets}
    (_P : ReservoirSitePlan A) : Fin (b + T) → Fin T → ZMod m :=
  fun _ _ => 0

noncomputable def baselineReservoirResidual
    {b m T : Nat} [NeZero m] [NeZero (m ^ b)]
    {packets : List (List Nat)}
    {A : OddSuccessorPhaseSplitBufferReservoirData
      (b := b) (m := m) (T := T) packets}
...
noncomputable def thresholdNonbufferCorrectionQuota
    ...
    ReservoirMoveToken A → ZMod m
  | ReservoirMoveToken.nonbuffer q =>
      P.baselineReservoirResidual initial
        (BaseTail.Trades.NonbufferReservoirToken.color q)
        (BaseTail.Trades.NonbufferReservoirToken.right hTpos q)
  | ReservoirMoveToken.buffer01 _ => 0
  | ReservoirMoveToken.buffer02 _ => 0

noncomputable def thresholdNonbufferCorrectionDelta
    {b m T : Nat} [NeZero m] [NeZero (m ^ b)]
    {packets : List (List Nat)}
    {A : OddSuccessorPhaseSplitBufferReservoirData
      (b := b) (m := m) (T := T) packets}
...
@[simp] theorem thresholdCanonicalQuota_nonbuffer_copy
    {b m T : Nat} [NeZero m] [NeZero (m ^ b)]
    {packets : List (List Nat)}
    {A : OddSuccessorPhaseSplitBufferReservoirData
      (b := b) (m := m) (T := T) packets}
    (P : ReservoirSitePlan A) (hT : 2 ≤ T)
    (initial : ActiveHall.Symboling A.Cyl.incidence)
    (a : A.nonbufferTokenBase) (k : Fin (m - 1)) :
    P.thresholdCanonicalQuota hT initial
        (ReservoirMoveToken.nonbuffer (a, k)) =
      P.baselineReservoirResidual initial a.1.1
        (nonbufferTokenBaseRight (A := A)
          (Nat.lt_of_lt_of_le (by omega : 0 < 2) hT) a) := rfl

@[simp] theorem thresholdCanonicalQuota_buffer01_copy
    {b m T : Nat} [NeZero m] [NeZero (m ^ b)]
    {packets : List (List Nat)}
    {A : OddSuccessorPhaseSplitBufferReservoirData
      (b := b) (m := m) (T := T) packets}
    ...
    (ρ : Fin (T - 1)) (k : Fin (m - 1)) :
    P.thresholdCanonicalQuota hT initial
        (ReservoirMoveToken.buffer01 (k, ρ)) =
      -P.postNonbufferReservoirResidual hT initial
        (A.buffer.color1 A.slotEquiv)
        (bufferTokenBaseRight
          (Nat.lt_of_lt_of_le (by omega : 0 < 2) hT) ρ) := rfl

@[simp] theorem thresholdCanonicalQuota_buffer02_copy
    {b m T : Nat} [NeZero m] [NeZero (m ^ b)]
    {packets : List (List Nat)}
    {A : OddSuccessorPhaseSplitBufferReservoirData
      (b := b) (m := m) (T := T) packets}
    ...
    (ρ : Fin (T - 1)) (k : Fin (m - 1)) :
    P.thresholdCanonicalQuota hT initial
        (ReservoirMoveToken.buffer02 (k, ρ)) =
      -P.postNonbufferReservoirResidual hT initial
        (A.buffer.color2 A.slotEquiv)
        (bufferTokenBaseRight
          (Nat.lt_of_lt_of_le (by omega : 0 < 2) hT) ρ) := rfl

theorem thresholdCanonicalQuota_nonbuffer_family_sum_const
    {b m T : Nat} [NeZero m] [NeZero (m ^ b)]
    {packets : List (List Nat)}
    {A : OddSuccessorPhaseSplitBufferReservoirData
      (b := b) (m := m) (T := T) packets}
    (P : ReservoirSitePlan A) (hT : 2 ≤ T)
    (initial : ActiveHall.Symboling A.Cyl.incidence)
    (weight : A.nonbufferTokenBase → ZMod m) :
    (∑ q : A.nonbufferTokens,
      ...
      ∑ a : A.nonbufferTokenBase,
        P.baselineReservoirResidual initial a.1.1
          (nonbufferTokenBaseRight (A := A)
            (Nat.lt_of_lt_of_le (by omega : 0 < 2) hT) a) *
          weight a := by
  exact
...
theorem thresholdCanonicalQuota_buffer01_family_sum_const
    {b m T : Nat} [NeZero m] [NeZero (m ^ b)]
    {packets : List (List Nat)}
    {A : OddSuccessorPhaseSplitBufferReservoirData
      (b := b) (m := m) (T := T) packets}
    ...
    (∑ q : BaseTail.Trades.BufferReservoirToken m T,
      ...
      ∑ ρ : Fin (T - 1),
        (-P.postNonbufferReservoirResidual hT initial
          ...
          weight ρ := by
  exact
    P.thresholdMoveTokenFinset_buffer01_family_sum_const
      (P.thresholdCanonicalQuota hT initial)
      (fun ρ =>
        -P.postNonbufferReservoirResidual hT initial
          (A.buffer.color1 A.slotEquiv)
...
theorem thresholdCanonicalQuota_buffer02_family_sum_const
    {b m T : Nat} [NeZero m] [NeZero (m ^ b)]
    {packets : List (List Nat)}
    {A : OddSuccessorPhaseSplitBufferReservoirData
      (b := b) (m := m) (T := T) packets}
    ...
    (∑ q : BaseTail.Trades.BufferReservoirToken m T,
      ...
      ∑ ρ : Fin (T - 1),
        (-P.postNonbufferReservoirResidual hT initial
          ...
          weight ρ := by
  exact
    P.thresholdMoveTokenFinset_buffer02_family_sum_const
      (P.thresholdCanonicalQuota hT initial)
      (fun ρ =>
        -P.postNonbufferReservoirResidual hT initial
          (A.buffer.color2 A.slotEquiv)
...
theorem exists_thresholdMoveCanonicalReservoirScript_of_exists_quota_nonzeroSolved
    {b m T : Nat} [NeZero m] [NeZero (m ^ b)]
    {packets : List (List Nat)}
    {A : OddSuccessorPhaseSplitBufferReservoirData
      (b := b) (m := m) (T := T) packets}
  ...
  let hTpos : 0 < T := Nat.lt_of_lt_of_le (by omega : 0 < 2) hT
  rcases P.exists_moveBaselineSymboling hTpos with
    ⟨Φ, hzeroToken, hrightToken⟩
  rcases hQuota Φ (by simpa [hTpos] using hzeroToken)
      (by simpa [hTpos] using hrightToken) with
    ⟨quota, hNonzeroSolved⟩
  let moves := P.thresholdMoveList hTpos quota
  let zeroColor := P.moveZeroColorOfMove hTpos
  let rightColor := P.moveRightColorOfMove hTpos
  have hpair :
      (BaseTail.Trades.nonzeroZeroSwapMovesOfTwoLe hT moves).Pairwise
        (fun move₁ move₂ => move₁.vertex ≠ move₂.vertex) := by
    simpa [hTpos, moves] using
      P.thresholdMoveList_swapMoves_pairwise hT quota
  have hzero :
      ∀ move, move ∈ moves →
        Φ.color move.vertex
            ⟨0, Nat.lt_of_lt_of_le (by omega : 0 < 2) hT⟩ =
    ...
    have hmem' :
        move ∈
          (P.thresholdMoveTokenFinset quota).toList.map
            (P.toNonzeroZeroSwapMove hTpos) := by
      simpa [hTpos, moves, thresholdMoveList] using hmem
    rcases List.mem_map.mp hmem' with
      ⟨q, _hqmem, rfl⟩
    simpa [hTpos, zeroColor, moveZeroColorOfMove_toNonzeroZeroSwapMove]
      using hzeroToken q
  have hright :
      ∀ move, move ∈ moves →
        Φ.color move.vertex move.right = rightColor move := by
    intro move hmem
    have hmem' :
        move ∈
          (P.thresholdMoveTokenFinset quota).toList.map
    ...
    rcases List.mem_map.mp hmem' with
      ⟨q, _hqmem, rfl⟩
    simpa [hTpos, rightColor, moveRightColorOfMove_toNonzeroZeroSwapMove]
      using hrightToken q
  have hCompat :=
    BaseTail.Trades.activeBlockResidueSpec_compatible_primitive
      A.isCylinder A.D hT
  have arithmetic :
    ...
    BaseTail.Trades.CanonicalNonzeroZeroReservoirArithmetic.ofNonzeroSolved
      ...
      (Φ.residueSpec_rowCompatible (m := m))
      (by
        simpa [hTpos, moves, zeroColor, rightColor] using hNonzeroSolved)
  exact
    ⟨BaseTail.Trades.CanonicalNonzeroZeroReservoirScript.ofArithmetic
      (D := A.D) (hT := hT) Φ moves zeroColor rightColor arithmetic
      hpair hzero hright,
      trivial⟩
