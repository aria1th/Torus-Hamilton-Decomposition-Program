...
DEFAULT_R42_CLOCK_CARRY = ROOT / "certs" / "routeE_r42_clock_carry_goal_audit.json"
...
def build_queue(
    ...
    r38_path: Path,
    r42_path: Path,
    r42_clock_carry_path: Path,
) -> dict[str, Any]:
    coverage = load(coverage_path)
    portfolio = load(portfolio_fits_path)
    r38 = load(r38_path)
    r42 = load(r42_path)
    r42_clock_carry = load(r42_clock_carry_path)
    ...
    for row in portfolio.get("rows", []):
        residue = int(row["residue"])
        status, action = classify_row(row, covered, r38_targets, r42, r42_clock_carry)
        rows.append(
            {
                "residue": residue,
                "status": status,
    ...
    return {
        ...
        "source_artifacts": {
            ...
            "r42_clock_carry_goal_audit": str(r42_clock_carry_path),
        },
        "residue_modulus": 48,
        "rows": rows,
        "summary": {
...
def main() -> None:
    ...
    parser.add_argument("--r42-clock-carry", type=Path, default=DEFAULT_R42_CLOCK_CARRY)
